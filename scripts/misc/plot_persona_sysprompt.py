#!/usr/bin/env python3
"""Plot persona system prompt ablation: OLMo vs generic system prompt."""
...
series = {
    "OLMo system prompt": f"{base}/hbsr_7b_persona_olmo_sysprompt/scores/7B-Persona-step{{s}}.jsonl",
    "Generic system prompt": f"{base}/hbsr_7b_persona_steps/scores_gpt5mini/7B-Persona-step{{s}}.jsonl",
}
...
apply_suptitle(fig, "System Prompt Effect on Eval Awareness\n(Persona Checkpoints)", fontsize=13, y=1.0)

out = os.path.join(os.path.dirname(__file__), "figs/persona_sysprompt_ablation.png")
