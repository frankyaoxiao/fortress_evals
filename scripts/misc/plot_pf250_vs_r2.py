#!/usr/bin/env python3
"""Bar graph: PersonaFrom250 vs R2 eval awareness over training steps."""
...
pf250_dir = "/home/fxiao/eval_awareness/fortress/runs/hbsr_7b_ifeval_persona_from250_generic_sysprompt/scores"
...
pf_steps, pf_rates, pf_los, pf_his = collect(pf250_dir, "7B-PersonaFrom250-step", range(275, 550, 25))
...
ax.bar(np.array(pf_x) + width/2, pf_vals, width, yerr=[pf_lo_v, pf_hi_v],
       ...
       label="Persona-From-250 (filtered from step 250)", alpha=0.85)
...
apply_suptitle(fig, "Eval Awareness: IFEval-Only vs Persona-From-250 RL (generic sysprompt)",
               fontsize=13, y=0.98)

out = os.path.join(os.path.dirname(__file__), "figs/pf250_vs_r2.png")
