#!/usr/bin/env python3
"""Bar graph: LLM-based filtering (EA) vs R2 eval awareness over training steps."""
...
ea_dir = "/home/fxiao/eval_awareness/fortress/runs/hbsr_7b_ifeval_ea_generic_sysprompt/scores"
...
r2_steps, r2_rates, r2_los, r2_his = collect(r2_dir, "7B-IFEvalOnly-R2-step", range(25, 550, 25))
ea_steps, ea_rates, ea_los, ea_his = collect(ea_dir, "7B-IFEvalEA-step", range(25, 550, 25))

all_steps = sorted(set(r2_steps) | set(ea_steps))
...
ea_x, ea_vals, ea_lo_v, ea_hi_v = [], [], [], []
for i, s in enumerate(all_steps):
    if s in ea_steps:
        idx = ea_steps.index(s)
        ea_x.append(i)
        ea_vals.append(ea_rates[idx])
        ea_lo_v.append(ea_los[idx])
        ea_hi_v.append(ea_his[idx])
...
ax.bar(np.array(ea_x) + width/2, ea_vals, width, yerr=[ea_lo_v, ea_hi_v],
       capsize=2, color=COLORS[2], edgecolor="white", linewidth=0.5,
       label="LLM-based filtering", alpha=0.85)
...
apply_suptitle(fig, "Eval Awareness: IFEval-Only vs LLM-based Filtering RL (generic sysprompt)",
               fontsize=13, y=0.98)

out = os.path.join(os.path.dirname(__file__), "figs/ea_vs_r2.png")
